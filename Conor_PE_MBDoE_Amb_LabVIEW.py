...
def PE_N_paramFive(filename, DiameterUsed, CatMassUsed, ObjectiveCriteria):
    ...
    n_theta = 4                            # number of unknown model parameters
    ...
    #ParametersGuess = [16, 6, 0.5, 0.5] # for LH model rate=k*CBA*CetOH/(1+Kw*Cw+KEtOH*CEtOH)^2
    ParametersGuess = [16, 6, 0.5, 0.5, 0.5, 0.5] # for LH model rate=k*CBA*CetOH/(1+Kw*Cw+KEtOH*CEtOH+KBA*CBA+KEB*CEB)^2
    ...
    def kinetic_model(c,W,u,theta):
        ...
        rate=1*(kpermass*CBA*CEtOH)/((1+KW*CW+KEtOH*CEtOH+KBA*CBA+KEB*CEB)**2)
        #rate=1*(kpermass*CBA*CEtOH)/((1+KW*CW+KEtOH*CEtOH)**2)               
    ...
    #wswrite['K'+str(row_count)]=params[3] #using the excel numbering system
    
    wswrite['L'+str(row_count)]=wt_residuals #using the excel numbering system
    wswrite['M'+str(row_count)]=chisq_ref #using the excel numbering system
    
    wswrite['N'+str(row_count)]=tvalues[0] #using the excel numbering system
    wswrite['O'+str(row_count)]=tvalues[1] #using the excel numbering system
    wswrite['P'+str(row_count)]=tvalues[2] #using the excel numbering system
    #wswrite['Q'+str(row_count)]=tvalues[3] #using the excel numbering system
    wswrite['R'+str(row_count)]=tref #using the excel numbering system
    
    wswrite['S'+str(row_count)]=ConfInt[0] #using the excel numbering system
    wswrite['T'+str(row_count)]=ConfInt[1] #using the excel numbering system
    wswrite['U'+str(row_count)]=ConfInt[2] #using the excel numbering system
    #wswrite['V'+str(row_count)]=ConfInt[3] #using the excel numbering system
    ...
    return NewDesign, params, ConfInt, tvalues, tref, wt_residuals, chisq_ref, Cov