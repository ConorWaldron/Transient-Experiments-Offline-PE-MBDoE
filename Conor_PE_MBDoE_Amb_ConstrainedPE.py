...
def PE_N_paramFive(filename, ObjectiveCriteria):
    ...
    ParameterRanges=np.zeros([n_theta,2])
    ParameterRanges[0,:]=[10, 20]
    ParameterRanges[1,:]=[4, 10]
    ParameterRanges[2,:]=[0.0, 3]
    ...
    def parameter_estimation(thetaguess, measureddata, inputconditions):
        ...
        #new_estimate = minimize(loglikelihood, thetaguess, method = 'SLSQP', bounds = ([ParameterRanges[0,0],ParameterRanges[0,1]],[ParameterRanges[1,0],ParameterRanges[1,1]],[ParameterRanges[2,0],ParameterRanges[2,1]]), options = {'maxiter':10000, 'ftol':1e-20}, args=(measureddata, inputconditions,))
