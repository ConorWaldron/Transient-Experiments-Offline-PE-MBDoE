...
FactorialParameters= [9.12, 7.98]  #for paper I used 9.1152 and 7.98194
GuessParameters= [9,8]
...
def chisquare_test(conf_level, ExpCond):
    dof = (((len(ExpCond)) * n_y) - len(GuessParameters))
    ref_chisquare = stats.chi2.ppf((conf_level),dof)
    return ref_chisquare
#chisq_ref = chisquare_test(confidence_level, SulfuricAcidData)
...
PreviousFixedDesign=[0.253, 29.8, 1.55, 119+273.15]
...
def Simulate100minExp(Design, Vr, Vd, SampleTime, theta, FixedDesign):
    alphaV=Design[0]
    vo=Design[1]
    FeedConc=Design[2]
    T=Design[3]
    alphaVB=FixedDesign[0]
    voB=FixedDesign[1]
    FeedConcB=FixedDesign[2]
    TB=FixedDesign[3]    
...
#testexp=Simulate100minExp(MBDoEDesign, ReactorVol, DeadVol, HPLCTime, FactorialParameters, PreviousFixedDesign)

def ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, epsilon, FixedDesign):
    #PastFisher=np.linalg.inv(PastCovariance)
    FutureExpData=Simulate100minExp(Design, Vr, Vd, SampleTime, TrueParams, FixedDesign)
    ...
    try:
        ExpectedCov=np.linalg.inv(TotalExpectedFisher)
            ...
            if 'Singular matrix' in str(err):
                ...
                raise
    return ExpectedCov
#TestExpCov=ExpectedCovFunctionforMultipleDataPoints(MBDoEDesign, ReactorVol, DeadVol, HPLCTime, FactorialParameters, Disturbance, PreviousFixedDesign )

def ObjFunction(Design, Vr, Vd, SampleTime, TrueParams, epsilon, Criteria, FixedDesign):
    if Criteria == "A":
        Obj=np.trace(ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, epsilon, FixedDesign))
    elif Criteria =="D":
        Obj=np.log(np.linalg.det(ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, epsilon, FixedDesign))) #we take log of determinant
    elif Criteria ==1:
        Obj=ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, epsilon, FixedDesign)[0,0]
    elif Criteria ==2:
        Obj=ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, epsilon, FixedDesign)[1,1]    
    else:
        e,v=np.linalg.eig(ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, epsilon, FixedDesign))    
        Obj=np.max(e)
    return Obj
TestObj=ObjFunction(MBDoEDesign, ReactorVol, DeadVol, HPLCTime, FactorialParameters, Disturbance, 'D', PreviousFixedDesign)
...
def FindBestGuess(ScreeningDesigns, Vr, Vd, SampleTime, TrueParams, epsilon, Criteria, FixedDesign):
    BestObjective=100
    BestDesign=0
    for i in range (0, len(ScreeningDesigns)): 
        CriteriaValue=ObjFunction(ScreeningDesigns[i,:], Vr, Vd, SampleTime, TrueParams, epsilon, Criteria, FixedDesign) #Problem in this line...
...
BestDesignFromScreening=FindBestGuess(Screening, ReactorVol, DeadVol, HPLCTime, FactorialParameters, Disturbance, 'D', PreviousFixedDesign)

def MBDoE(NewExp, Vr, Vd, SampleTime, TrueParams, epsilon, Criteria, FixedDesign):
    ...
    return new_design    
MinimisedMBDoE=MBDoE(BestDesignFromScreening, ReactorVol, DeadVol, HPLCTime, FactorialParameters, Disturbance, 'D', PreviousFixedDesign)
NewDesign=MinimisedMBDoE.x
Objvalue=MinimisedMBDoE.fun


NewExperiment=Simulate100minExp(NewDesign, ReactorVol, DeadVol, HPLCTime, FactorialParameters, PreviousFixedDesign)
...
wswrite['L'+str(2)]=PreviousFixedDesign[0]
wswrite['L'+str(3)]=PreviousFixedDesign[1]
wswrite['L'+str(4)]=PreviousFixedDesign [2]
wswrite['L'+str(5)]=PreviousFixedDesign[3]
wswrite['L'+str(1)]="Fixed Design, alpha V, v0, C, T" #using the excel numbering system 
wswrite['M'+str(1)]="Parameters used for MBDoE" 
wswrite['M'+str(2)]=FactorialParameters[0]
wswrite['M'+str(3)]=FactorialParameters[1]
...
#Saving File
wbwrite.save("Redesign ???.xlsx")# overwrites without warning. So be careful
