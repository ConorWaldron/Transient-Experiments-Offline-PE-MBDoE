...
def ImportData(filename):
    ...
    return datamatrix    
#SulfuricAcidData=ImportData('Dec 14C Python Format Corrected.xlsx')
...
#SulfuricAcidData=ImportManyFiles('Dec 12 Python Format Corrected.xlsx', 'Dec 14C Python Format Corrected.xlsx')
...
#check=SimulateSingleIsothermalDataPoint(KnownParameters, 1.5, 413.15, 100, 1, testtime, ReactorVol, DeadVol)
...
CheckGeneratedSol=generatepredictions(SulfuricAcidData, KnownParameters, ReactorVol, DeadVol)
...
####################################################################################################################################
'''
#I need to start by picking a new experimental condition
#guessInitalFlow=100
#guessFlowRamp = 0.5
#guessConstantFeedConc = 1.25
#guessConstantTemp=400
#guessExpDesign=[guessInitalFlow, guessFlowRamp, guessConstantFeedConc, guessConstantTemp]

#Here I start with the easier situation where I fix all variables excpet flowramprate
FixInitialFlow = 50
FixAlphaV      = 1.71
FixFeedConc    = 1.25
FixTemp        = 130+273.15
...
#testexp=Simulate100minExp([0.00001,2,1.25,400], ReactorVol, DeadVol, HPLCTime, params)
...
def ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon ):
    ...
    return ExpectedCov
#TestExpCov=ExpectedCovFunctionforMultipleDataPoints(MBDoEDesign, ReactorVol, DeadVol, HPLCTime, params, Cov, Disturbance )

def ObjFunction(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon, Criteria):
    if Criteria == "A":
        Obj=np.trace(ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon))
    elif Criteria =="D":
        Obj=np.log(np.linalg.det(ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon))) #we take log of determinant
    elif Criteria ==1:
        Obj=ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon)[0,0]
    elif Criteria ==2:
        Obj=ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon)[1,1]    
    else:
        e,v=np.linalg.eig(ExpectedCovFunctionforMultipleDataPoints(Design, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon))    
        Obj=np.max(e)
    return Obj
#TestObj=ObjFunction(MBDoEDesign, ReactorVol, DeadVol, HPLCTime, params, Cov, Disturbance, 'D' )
...
def FindBestGuess(ScreeningDesigns, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon, Criteria):
    BestObjective=100
    BestDesign=0
    for i in range (0, len(ScreeningDesigns)): 
        CriteriaValue=ObjFunction(ScreeningDesigns[i,:], Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon, Criteria) #Problem in this line...
...
BestDesignFromScreening=FindBestGuess(Screening, ReactorVol, DeadVol, HPLCTime, params, Cov, Disturbance, 'D')

def MBDoE(NewExp, Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon, Criteria):
    new_design=minimize(ObjFunction, NewExp, method = 'SLSQP', bounds = ([Variableranges[0,0],Variableranges[0,1]],[Variableranges[1,0],Variableranges[1,1]],[Variableranges[2,0],Variableranges[2,1]],[Variableranges[3,0],Variableranges[3,1]]), options = {'maxiter':10000, 'ftol':1e-20}, args = (Vr, Vd, SampleTime, TrueParams, PastCovariance, epsilon, Criteria,))
    return new_design    
MinimisedMBDoE=MBDoE(BestDesignFromScreening, ReactorVol, DeadVol, HPLCTime, params, Cov, Disturbance, 'D')
NewDesign=MinimisedMBDoE.x
Objvalue=MinimisedMBDoE.fun
'''
'''

NewExperiment=Simulate100minExp(NewDesign, ReactorVol, DeadVol, HPLCTime, params)
#rivalDesign=[5.10344236e-02,   5.1e+00,   1.55000000e+00, 4.03436338e+02]
#NewExperiment=Simulate100minExp(rivalDesign, ReactorVol, DeadVol, HPLCTime, params)
wbwrite=openpyxl.Workbook()
wswrite=wbwrite.active #opens the first sheet in the wb
