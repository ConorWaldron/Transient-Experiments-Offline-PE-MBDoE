...
def kinetic_model_C(c,W,u,theta):
    CBA = c[0] #mol/L
    CEtOH = c[1]
    CEB = c[2] #mol/L
    CW = c[3]
...
def MB_3Models_ObjFunction(SinlgeExp, ThetaA, ThetaB, ThetaC, stdev):
    PredictedConcA=SingleExpGeneratePredictions(SinlgeExp, ThetaA, kinetic_model_A)
    PredictedConcB=SingleExpGeneratePredictions(SinlgeExp, ThetaB, kinetic_model_B)
    PredictedConcC=SingleExpGeneratePredictions(SinlgeExp, ThetaC, kinetic_model_C)
    DeviationAB = np.zeros(len(stdev))
    DeviationAC = np.zeros(len(stdev))
    DeviationBC = np.zeros(len(stdev))
    DeviationVarianceSqAB = np.zeros(len(stdev))
    DeviationVarianceSqBC = np.zeros(len(stdev))
    DeviationVarianceSqAC = np.zeros(len(stdev))
    for i in range(len(stdev)):
        DeviationAB[i]=PredictedConcA[i]-PredictedConcB[i]
        DeviationAC[i]=PredictedConcA[i]-PredictedConcC[i]
        DeviationBC[i]=PredictedConcB[i]-PredictedConcC[i]
        
        DeviationVarianceSqAB[i]=(DeviationAB[i]/stdev[i])**2
        DeviationVarianceSqAC[i]=(DeviationAC[i]/stdev[i])**2
        DeviationVarianceSqBC[i]=(DeviationBC[i]/stdev[i])**2

    SumDeviationsSqAB=sum(DeviationVarianceSqAB)
    SumDeviationsSqAC=sum(DeviationVarianceSqAC)
    SumDeviationsSqBC=sum(DeviationVarianceSqBC)
    
    Objective=-(SumDeviationsSqAB + SumDeviationsSqAC + SumDeviationsSqBC)  #I set it to negative so I can still minimise.
    return Objective
    
def MD(filename, DiameterUsed, CatMassUsed):
    start=time.time()
    #load data
    ...
    minimisedlogC=parameter_estimation(GuessParameters_modelC, y_m, exp_conditions, sigma, kinetic_model_C)
    paramsC = minimisedlogC.x
    wt_residualsC = 2*minimisedlogC.fun    
    ...
    def FindBestGuess(Designs, ThetaA, ThetaB, ThetaC, stdev):
        BestObjective=100
        BestDesign=0
        for i in range (0, len(Designs)):
            CriteriaValue=MB_3Models_ObjFunction(Designs[i], ThetaA, ThetaB, ThetaC, stdev)
    ...
    BestDesignFromScreening=FindBestGuess(Screening, paramsA, paramsB, paramsC, sigma)
                            
    def MD_Optimisation(NewExp, ThetaA, ThetaB, ThetaC, stdev):
        new_design=minimize(MB_3Models_ObjFunction, NewExp, method = 'SLSQP', bounds = ([Variableranges[0,0],Variableranges[0,1]],[Variableranges[1,0],Variableranges[1,1]],[Variableranges[2,0],Variableranges[2,1]],[DiameterUsed,DiameterUsed],[CatMassUsed,CatMassUsed]), options = {'maxiter':10000, 'ftol':1e-20}, args = (ThetaA, ThetaB, ThetaC, stdev,))
        return new_design    
    MinimisedMBDoE=MD_Optimisation(BestDesignFromScreening, paramsA, paramsB, paramsC, sigma)
    ...
    return NewDesign, paramsA, wt_residualsA, paramsB, wt_residualsB, paramsC, wt_residualsC
