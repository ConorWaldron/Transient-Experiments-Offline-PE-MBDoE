...
FactorialParameters= [9.1152, 7.98194]
GuessParameters= [9,8]
...
#SulfuricAcidData=ImportData('RampFandT bad design.xlsx')
#SulfuricAcidData=ImportData('RampFandT good design.xlsx')
SulfuricAcidData=ImportData('Dec 14D Python Format Corrected.xlsx')    #Dec 14D Python Format Corrected.xlsx is the file containing the results of the actually conducted MBDoE Ramp FT experiment, we load it here to double check the parameter estimation
...
testtimeresidence=Times(10.1, 0.05, ReactorVol, DeadVol, 5876)
'''
#Isothermal functions below are not used in this code
...
#check=SimulateSingleIsothermalDataPoint(FactorialParameters, 1.5, 413.15, 100, 1, 5876, ReactorVol, DeadVol)
'''

#these are the functions used in this code
...
def SimulateSingleTvaryingDataPoint(theta, FeedConc, T0, alphaT, vo, alphaV, tmSec, Vr, Vd):
    CBenzoicAcid=np.zeros(5)
    CEthylBenzoate=np.zeros(5)
    ...
    timepoints=np.linspace(0,residencetime,100)
    InitalTforthisSample=T0-(alphaT/60)*timeenter
    ...
    CEthylBenzoate[0]=FeedConc-CBenzoicAcid[0]
    return CBenzoicAcid[0], CEthylBenzoate[0]
#CheckGeneratedSol=SimulateSingleTvaryingDataPoint(FactorialParameters, 1.556, 412.35, 0.537, 10.1, 0.05, 5876, ReactorVol, DeadVol)

def generatepredictions(ExpCond, theta, Vr, Vd):
    ConcPredicted=np.zeros([len(ExpCond),2])
    for i in range (0, len(ExpCond)):
        soln = SimulateSingleTvaryingDataPoint(theta, ExpCond[i,3], ExpCond[i,6], ExpCond[i,7], ExpCond[i,4], ExpCond[i,5], ExpCond[i,0], Vr, Vd)
        CBA = soln[0]
        CEB = soln[1]
        ConcPredicted[i,:] = [CBA, CEB]
    return ConcPredicted

SimulatingParameters=[9.11, 7.98]
CheckGeneratedSol=generatepredictions(SulfuricAcidData, SimulatingParameters, ReactorVol, DeadVol)

#Output prediction to excel,                     I wrote this code so I could generate predictions for each experiment using different parameter estimates
...
#Saving File
wbwrite.save("PredictedConc_SS_FAct.xlsx")# overwrites without warning. So be careful
...
def loglikelihood(theta, ExpCond, Vr, Vd):
    ...
    return obj_fun
CheckLog=loglikelihood(GuessParameters, SulfuricAcidData, ReactorVol, DeadVol)

##################################################################################################
#Note that this parameter estimation is not used for desinging new experiments, it is just here for preforming parameter estimation, after the experiment is run and the data is collected
##################################################################################################
...
def sensitivity(OneExp, epsilon, TrueParams, Vr, Vd):
    ...
    for i in range(len(KPMatrix)):
        Solution = SimulateSingleTvaryingDataPoint(KPMatrix[i,:], OneExp[3], OneExp[6], OneExp[7], OneExp[4], OneExp[5], OneExp[0], Vr, Vd)
...
####################################################################################################################################
#Here I start MBDoE for isothermal flowrate ramps, note none of the parameter estimation above is needed here. This code has two independent parts, parameter estimation and MBDoE
##################################################################################################################################


#Define a first guess for the MBDoE design
FixFeedConcA    = 1.25 #mol/L
...
MBDoEDesign    =[FixAlphaVA, FixInitialFlowA, FixFeedConcA, FixTempA, FixAlphaTA]
#Here we define the variable range for our designed variables.
Variableranges=np.zeros([5,2])
...
#We do not use previous information for designing this experiment, so the expected covariance is just the new exp, no contribution from old experiments
...
def FindBestGuess(ScreeningDesigns, Vr, Vd, SampleTime, TrueParams, epsilon, Criteria):
    BestObjective=999999  #start with a high number so the designs will be better than this
    BestDesign=0
...
def MBDoE(NewExp, Vr, Vd, SampleTime, TrueParams, epsilon, Criteria):
    new_design=minimize(ObjFunction, NewExp, method = 'SLSQP', bounds = ([Variableranges[0,0],Variableranges[0,1]],[Variableranges[1,0],Variableranges[1,1]],[Variableranges[2,0],Variableranges[2,1]],[Variableranges[3,0],Variableranges[3,1]],[Variableranges[4,0],Variableranges[4,1]]), options = {'maxiter':10000, 'ftol':1e-20}, args = (Vr, Vd, SampleTime, TrueParams, epsilon, Criteria,))
    return new_design    
...
'''
#Here we check if the optimum design is better than other designs

NewExperimentOpt=Simulate100minExp(NewDesign, ReactorVol, DeadVol, HPLCTime, FactorialParameters)
NewExperimentScreen=Simulate100minExp(BestDesignFromScreening, ReactorVol, DeadVol, HPLCTime, FactorialParameters)

rivalDesign=[1.12459865e-02,   6.89082105e+00,   1.14570947e+00, 4.03928682e+02,   9.98827497e-01]
...
def savefunction(NewExperiment, WriteFile):
    ...
    return     
#after saving these files we can use this same code to run parameter estimation
saveOpt=savefunction(NewExperimentOpt, 'OptRampFandT10000')
saveScreen=savefunction(NewExperimentScreen, 'ScreenRampFandT10000')
'''
